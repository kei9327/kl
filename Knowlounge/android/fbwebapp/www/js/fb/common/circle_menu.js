;(function($, window, document, undefined){
	...
	CircleMenu.prototype.init = function(){
		...
		if(self.options.depth > 1){
			self.options.item_diameter = self.options["depth" + (self.options.depth)].item_diameter;
			self.options.circle_radius = self.options["depth" + (self.options.depth)].circle_radius;
			self.options.angle.start = self.options["depth" + (self.options.depth)]["angle" + self.item_count].start;
			self.options.angle.end  = self.options["depth" + (self.options.depth)]["angle" + self.item_count].end;	
		}
