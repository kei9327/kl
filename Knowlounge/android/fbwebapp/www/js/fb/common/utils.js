var UrlResource = {
	"domain.svr.0" : "http://fb.wenote.com/fb/",
	"domain.svr.1" : "https://www.wenote.com/fb/",
	...
	"profile.default" : "res/fb/images/thum_user.png",
...
var Utils = {
	...
	isCordova : false,
	log : function(msg){
		if(location.hostname == "test.wenote.com" || location.hostname == "fb.wenote.com") console.log(msg);
//		console.log(msg);
	},
	...
	addResPath : function(type, url){
		return this.addContext("res/" + type + "/" + url);
	},
	...
	request : function(url, type, data, onsuccess, onfailure, oncreate, oncomplete){
		
		var tmpSuccess = function(data) {
			var flag = false;
			var ckData = data;
			if("html" == type.toLowerCase()) {
				try {
					if(data && data != null && data != ""){
						data = data.trim();
					}			
					ckData = jQuery.parseJSON(data);
					flag = true;
				}catch(e){
					// json convert 실패시 string 이므로 정상
					flag = false;
				}
			}else if("json" == type.toLowerCase()){
				
			}else{
				// 잘못된 케이스 html or json이 아닌 경우, 잘못된 CASE 1
			}

			if(ckData != null && flag){	// html string으로 들어온 경우 통과..
				var result = 0;
				if(ckData.result){
					result = ckData.result					
				}else{
					result = -200;
				}   
				
				var exceptionUrl = Utils.addContext(_url("exception.url"));
										
				if(result == -100){			// data connect error
					exceptionUrl += "/100";
					location.href = exceptionUrl;
					return;
				}else if(result == 404){	// not found
					exceptionUrl += "/404";
					location.href = exceptionUrl; 
					return;
				}else if(result == 500){	// jsp exception
					exceptionUrl += "/500";					
					location.href = exceptionUrl;
					return;
				}else if(result == 600){	// not login ajax exception
					//exceptionUrl = Utils.addContext(_url("login"));
					exceptionUrl = ckData.url;					
					location.href = exceptionUrl;
					return;
				}
			}
		...
		if(onfailure == null){
			onfailure = function(){		
				// 2011.07.14 제거 		
				// top.document.location.href = Utils.addContext(_url("exception.url")) + "/101";
				return;
			}
		}
	...
	resizeAfterMove : function(fn, readyFn, timeout){ 
	    ...
	    var screenX=0, screenY=0, fixCnt = 0;
	    var timer = null;
...
/**
//resize end check
var onResize = function(fn, readyFn, timeout) {    
    ...
    $(window).on('resize', function(e) {
        ...
        onResize.timeout = window.setTimeout("onResize.loop();", timeout);
    });    
};
**/
