			...
			this.load(pageId, function(data){
				...
				*/
				...
				if(background){
					...
					var r = background.RED ? background.RED : "";
					var g = background.GREEN ? background.GREEN : "";
					var b = background.BLUE ? background.BLUE : "";
					var bgImg = background.PLUGINDATA ? background.PLUGINDATA : ""; 
				...
				for(var i=0; i<len; i++){
					console.log("[UI.Page.load] callback() - 이미지/PDF 리스트 로딩..");
				...
				// 3. memo packet
				if(memo != null){
					console.log("[UI.Page.load] callback() - 메모 리스트 로딩..");
...
1					UI.Page.change(pageId);
		...
		remove : function(pageId){
			...
			Ctrl.Modal.confirm(_msg("confirm.page.remove"), function(){
				...
				var svrFlag = _prop("svr.flag");
				var svrHost = _prop("svr.host." + svrFlag);
				var url = svrHost + _url("page.remove");
...
var PacketMgrCordova = {
	...
	_drawPagePacket : function(pageId, isAnimation){
		...
		if(PacketMgr.pageMap != null){
			...
			if(packetMap != null){
				var dataKeys = PacketMgr.pageMap.keys();
					...
					for(var i=0; i<len; i++){
						...
						if(isAnimation){
							...
							setTimeout(function(){
								...
								if(json != null) PacketMgr.receiveForSync(json, pageId, isAnimation);	
							...
							PacketMgr.receiveForSync(packet, pageId, isAnimation);
						}
...
// // console.log("packet : " + JSON.stringify(packet));
	...
	*/
	_drawHistoryPacket : function(pageId, isAnimation, callback){		
		...
		if(PacketMgr.pageMap != null){			
			console.log("[PacketMgr._drawHistoryPacket] Page 패킷이 발견되어 페이지의 패킷을 로딩합니다..");
			...
			if(packetMap != null){
				...
				for(var i=len-1; i >= 0; i--){
					...
					if(cmd == "zoom"){
						...
						}, "RoomPlugin", "setZoomVal", [{"zoom" : zoomVal}]);
    ...
    Command : {
        ...
        masterchange : function(packet) {
            ...
            Ctrl.BGImg.auth();    // bgimage auth
            
            Ctrl.Memo.auth();    // memo event reset
            
			// 영상 공유
			Ctrl.VShare.auth();

            Ctrl._usePlugin(PacketMgr.isMC);  // slider enable
        ...
        poll : function(packet) {
			...
			console.log("[PacketMgr.Command.poll] packet : " + JSON.stringify(packet));
            ...
            if(type == 'start') {
				...
				// timeLimit 후에 isPollProcessing 플래그를 false로 비활성화 시킴..
				setTimeout(function(){
				...
				Ctrl.isPollProcessing = false;  // 폴이 중단되었을 때 예외처리 플래그값 비활성화
            } else if(type == 'report') {
		...
		sync : function(packet){
			...
			if(method != null && method != ""){
				...
				/*
				// 웹 방식이므로 주석처리 - 2016.07.02
				if(SyncUpdated[fName] != null && typeof(SyncUpdated[fName]) == "function"){
					SyncUpdated[fName].call();
				}
				*/
...
var CtrlCordova = {
    ...
    colorMap : [
                {r:0,g:100,b:250},   // 1번 컬러
                {r:100,g:0,b:250},   // 2번 컬러
                {r:250,g:50,b:50},   // 3번 컬러
                {r:250,g:100,b:0},   // 4번 컬러
                {r:50,g:50,b:50},    // 5번 컬러
                {r:0,g:200,b:250},   // 6번 컬러
                {r:200,g:0,b:250},   // 7번 컬러
                {r:250,g:100,b:100}, // 8번 컬러
                {r:250,g:200,b:0},   // 9번 컬러
                {r:-1,g:-1,b:-1},    // 10번
                {r:-1,g:-1,b:-1}     // 11번
                ],
    ...
    pointColorMap : [
                     ...
                     {r:250,g:50,b:50},   // 3번 컬러
                     {r:250,g:100,b:0},   // 4번 컬러
                     {r:0,g:250,b:100},   // 5번 컬러
                     {r:0,g:200,b:250},   // 6번 컬러
                     {r:200,g:0,b:250},   // 7번 컬러
                     {r:250,g:100,b:100}, // 8번 컬러
                     {r:250,g:200,b:0},   // 9번 컬러
                     {r:0,g:250,b:200}   // 10번 컬러
                     ],
    ...
    Text : {
		...
		/**
		 * Ctrl.Text._setTextEvent : 재정의
		 *
		 */
    	_setTextEvent : function(){
			console.log("[_setTextEvent]");
            ...
            $("#sketch" + UI.current).click(function(ev){
				...
				console.log("Ctrl.Text.isOpened() : " + Ctrl.Text.isOpened() + ", Ctrl.Text.current : " + Ctrl.Text.current + ". Ctrl.Text.isBlank : " + Ctrl.Text.isBlank() );
                ...
                console.log("[_setTextEvent] x : " + x + ", y : " + y);
...
//						posY += (top - 10);
    ...
    Member : {
		...
		**/
		...
		/**
         * Ctrl.Member._removeClass : 재정의 함
         * 클래스 유저 삭제
         */
        ...
        /**
         * Ctrl.Member.addRoomUser : 신규 정의함
         * RoomSvr.newuser에서 호출함..
         * 새로 들어오는 유저 정보를 ListView에 출력할 때 호출하는 함수..
         */
        addRoomUser : function(userId, userName, userNo, guest, thumbnail, userType, isVideoAllow) {
            ...
            // member 추가
            Ctrl.Member._add(userId, userName, userNo, guest, thumbnail);
...
//            if(CanvasApp.info.userno != userNo)  // 새로 들어온 유저에 대해서만 리스트에 뿌리기 위한 조건문
        ...
        /**
         * Ctrl.Member.removeRoomUser : 신규 정의함
         ...
         */
        addClassUser : function(roomId, userId, userName, userNo, isGuest, thumbnail, userRoomId, userRoomSeqNo, userType) {
            var cordovaParams = [{
                //roomid    : roomId,
                userno    : userNo,
                userid    : userId,
        ...
        masterChange : function(userId){
			...
			}, "UserPlugin", "changeMaster", [{userid : userId}]);
            ...
            if(cordova.platformId == "android") {
				...
				}, "VideoPlugin", "changeMaster", [{userid : userId}]);
			}
...
//                });
        ...
        moveMyRoom : function(cookieStr) {
            ...
            $.ajax({
                ...
                success : function(data) {
                    console.log(data);
                    if(data.result == '0') {
                        var roomUrl = "view.html?code=" + data.code;
                        setTimeout(function(){
                            cordova.exec(function(result){
                            }, function(result){}, "RoomPlugin", "moveRoom", [roomUrl]);
                        }, "1500");
                    } else {
    ...
    Modal : {
		...
		confirm : function(msg, callback){
			...
			var cordovaParam = [{ body_message : msg }];
        ...
        remove : function(commentNo) {
            ...
            Utils.request(url, "json", params,  function(json){
                if(json.result == 0){
                    ...
                    var commentParam = [{ commentno : commentNo }];
...
//            }
    ...
    Uploader : {
        ...
        setImageUploadEvent : function() {
            ...
            if(platform.toLowerCase() === 'android') {
				...
				filechooser.open({"mime_type" : "image/*"}, function(data){
                    console.log(data);
        ...
        setPdfUploadEvent : function() {
            ...
            if( platform.toLowerCase() === 'android') {
				...
				filechooser.open({"mime_type" : "application/pdf"}, function(data){
					console.log(data);
					...
					var blobContent = new Blob([byteContent], {type: data.mimetype});
        ...
        uploadForiOS : function(fileName, type){
            ...
            function gotFS(fileSystem) {
                fileSystem.root.getDirectory("/tmpDownload", {create: false, exclusive: false}, gotDir, fail);
            }
            function gotDir(dir) {
    ...
    Room : {
        ...
        update : function(jsonStr){
            ...
            var ckUsePasswd  = (jsonParam.ckUsePasswd != null) ? jsonParam.ckUsePasswd : false;                                               // 수업 비밀번호 존재 여부
            var passwdTxt    = (jsonParam.passwdTxt != null) ? jsonParam.ckUsePasswd : (curRoomInfo != null ) ? curRoomInfo.passwd : "";   // 수업 비밀번호
			...
			*/
            ...
            PacketMgr.Master.updateRoomInfo(ckAuthType, chatOpt, cmtOpt, expOpt, title, passwdTxt, openFlag, content);
        ...
        updateRoomInfo : function(packet) {
            console.log("updateRoomInfo / " + JSON.stringify(packet));
		...
		renderVShareList : function(vShareList, callback) {
            ...
            if(len > 0) {
                for(var i=0; i<len; i++) {
					...
					var htmlStr = "<div class=\"videoImportWrap vshare addVideo\" seqno=\""+seqNo+"\" vsno=\""+commentNo+"\" ord=\""+ord+"\" style=\"visibility:hidden; left:"+posX+"px; top:"+posY+"px; z-index:"+zIndex+"\">\
...
//        }, "1500");
    ...
    goMainPage : function() {
        cordova.exec(function(result){
        }, function(result){}, "RoomPlugin", "exitRoom", [{}]);
    },
	...
	**/
    bindCordovaEvents: function() {
        ...
        document.addEventListener('deviceready', function() {
            ...
            var codeStr = parseQueryString(location.search.slice(1)).code;
            console.log(codeStr);
...
//        }
    ...
    getRoomInfo : function(codeStr, device, userInfoParams) {
        ...
        if(tokenStr == null) {
            console.log("[CanvasApp.getRoomInfo] token encrypt fail..");
        ...
        Utils.request(url, "json", param, function(data) {
            if(data.result == '0') {
				...
				var pageArr = [];
				var pageLen = info.pageList.length;
				for(var i=0; i<pageLen; i++) {
					pageArr.push(info.pageList[i].pageid);
				}

				// 캔버스 페이지 리스트를 네이티브로 전달함.. - 2016.10.19
				if(info.pageList != null && info.pageList.length > 0) {
					cordova.exec(function(result) {
						console.log("cordova.exec() success.. initPageList");
					}, function(result) {
						console.log("initPageList error : " + JSON.stringify(result));
					}, "MultiPagePlugin", "initPageList", pageArr);
				}
                ...
                var currentRoomInfoParams = {
                    bookmark		: info.bookmark,
                    roomid			: info.roomid,
                    roomtitle		: info.roomtitle,
                    creatorflag	    : info.userid == info.creatorid ? true : false,
                    masterflag	    : info.userid == info.masterid ? true : false,
                    guestflag	    : info.userid == info.userno ? true : false,
                    masterid	    : info.masterid,
                    masterno     	: info.masterno,
					creatorno       : info.creatorno,
                    masterseqno	    : info.masterseqno,
					parentcreatorno : info.parentcreatorno,
                    userno          : info.userno,
                    userid		    : info.userid,
                    usernm		    : info.usernm,
                    auth		    : info.auth,
                    bg			    : info.bg,
                    snstype		    : userInfoJson.snstype == undefined ? "" : userInfoJson.snstype,
					userlimitcnt    : info.auth.userlimitcnt,
					currentpageid   : info.currentpageid,
					codestr         : codeStr   // IOS 때문에 추가됨.. 2016.08.17
                };

                console.log("[CanvasApp.getRoomInfo] 네이티브로 전달할 룸 초기 정보(currentRoomInfoParams) : " + JSON.stringify(currentRoomInfoParams));

                // Native 메뉴에 init 하는 플러그인 호출
                cordova.exec(function(result) {
                    console.log("cordova.exec() success.. initializeRoom");
                }, function(result){
                    console.log("initializeRoom error : " + JSON.stringify(result));
                }, "RoomPlugin", "initializeRoom", [currentRoomInfoParams]);
                    ...
                    PacketMgr._drawHistoryPacket(PacketMgr.lastFileNo, PacketMgr.lastPageNo, isAnimation, function() {
                        ...
                        if(!isAnimation) {
                            ...
                            cordova.exec(function(result) {
                                ...
                                alert('finishRoomLoading err '+JSON.stringify(result));
                            }, "myPlugin", "finishRoomLoading", []);
                        }
                    });
					**/
	...
	**/
	redrawHistory : function(){
		...
		var cordovaParam = {
			redraw_enable : CanvasApp.confirmAction
		};
		cordova.exec(function(result) {
...
//        if(Utils.isKLounge) {
    ...
    /**
     * RoomSvr.connect (재정의)
     *  내가 룸에 입장했을 때 호출되는 함수. RoomSvr.init(info) 시점에서 호출됨.
     */
    connect : function() {
        console.log("[RoomSvr.connect] Socket.IO - " + RoomSvr.userid + ", " + RoomSvr.userno + ", " + RoomSvr.usernm + ", " + RoomSvr.deviceid);
    ...
    sendbroadcast : function(mode, roomId, data){
		console.log("sendbroadcast : " + mode + ", roomId : " + roomId + ", data : " + data);
     ...
     */
    newuser : function(userid, username, userno, guest, thumbnail, userType, isvideoallow) {
		...
		*/
        ...
        Ctrl.Member.addRoomUser(userid, username, userno, guest, thumbnail, userType, isvideoallow);
    },
    ...
    leaveuser : function(userId, userName, userNo) {
        ...
        console.log("[RoomSvr.leaveuser] Socket.IO - userId : " + userId + ", userName : " + userName);
	 ...
	 *   - userType      : 유저 타입  (0 - 일반, 1 - 학생, 2 - 선생님)
     */
    newclassuser : function(roomId, userId, userName, userNo, isGuest, thumbnail, userRoomId, userRoomSeqNo, userType){
        ...
        //if(Utils.isKLounge) {
        if(true) {
			var newClassUserInfo = Ctrl.Member.getClass(userNo, "userno");
			console.log("[RoomSvr/newclassuser] newClassUserInfo : " + JSON.stringify(newClassUserInfo));
            if(RoomSvr.userno != userNo && newClassUserInfo != null){
				console.log("[RoomSvr/newclassuser] RoomSvr.userno : " + RoomSvr.userno);
                return;
            }

            Ctrl.Member.addClassUser(roomId, userId, userName, userNo, isGuest, thumbnail, userRoomId, userRoomSeqNo, userType);
        }
    },
    ...
    kickuser : function(roomid, userno, userid, username, kicktype){
        console.log("leaveuser kickuser roomid : " + roomid + ", userid : " + userid + ", username : " + username + ", kicktype : " + kicktype);
        ...
        if(RoomSvr.roomid == roomid){
            if(PacketMgr.userid == userid) {
				...
				console.log("leaveuser kickuser !!!!", userid, username, userno);
     ...
     */
    newvc : function(roomId, user1, user2, data, user1_video_allow, user2_video_allow){
	 ...
	 *   - user2_video_allow : 상대방의 영상 허용여부
     */
	newvcf : function(roomId, user1, user2, data, user1_video_allow, user2_video_allow){
...
$(document).ready(function() {	
	window.onbeforeunload = function(){
		alert("onbeforeunload event fired");
		return;
	};
});